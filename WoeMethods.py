...
class bins_method_funcs(object):
    def __init__(self, df, methods = 'tree', pct_size = 0.05, max_grps = 5, chiq_pv = 0.05, cuts = None, ifmono = True):
        """
...
#        self.mono_bins = None
    ...
    def freq_bins_funcs(self):
        ...
        self.bins = {ft_name:rlts+[max(prm_cuts)]}
        self.cap_info = {'max': tmp[ft_name].max(), 'min':tmp[ft_name].min()}
    ...
    def woe_cal(self, data = None, ifmono = True, ifiv = False, ifnan = True, code = True):
        """
        计算特征的IV值及相应分箱的WOE编码
    ...
    def woe_apply(self, data = None, cuts = None, woe_info = None, mothed = 'mono', ifna = True):
        """
        用WOE编码替换分组信息
        """
        if data is None:
